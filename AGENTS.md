...
- `npm run dev:task:stop` 必须同时停止由该任务管理的前端与任务后端，并让 `5175` 自动回退到 `5173` 集成环境；不得留下旧任务页面或 API 已失效的入口。
...
- GitHub 插件用于仓库、PR、Issue 和评审等只读查询；当前安装对本仓库 PR 写操作返回 `403 Resource not accessible by integration`，创建、更新、评论或关闭 PR 直接使用已认证的 `gh` CLI，不先重复尝试插件。代码分支推送继续使用 Git SSH。
