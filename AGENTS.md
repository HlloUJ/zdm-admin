...
- 正式联合验收使用从准确候选清单创建的临时 `codex/acceptance-*`，不得直接复制不断变化的集成分支状态；验收缺陷从验收使用的准确提交创建修复分支，不带入同模块后续未验收功能。
...
- 任务分支验收前同步最新 `origin/main`；联合验收、合并或高风险回归运行 `npm run verify` 或 `npm run verify:local`。
