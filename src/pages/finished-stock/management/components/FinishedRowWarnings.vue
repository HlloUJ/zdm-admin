<template>
  <div ref="root" class="finished-row-warnings">
    <slot />
    <t-alert
      v-for="item in positions"
      :key="item.id"
      class="finished-row-warning"
      :style="item.style"
      theme="warning"
...
<style scoped>
.finished-row-warnings {
  position: relative;
}
.finished-row-warnings :deep(tr[data-shelf-error-id] > td) {
  padding-top: 40px;
}
.finished-row-warning {
  position: absolute;
  z-index: 31;
...
.finished-row-warning :deep(.t-alert__message) {
  font-size: 11px;
  line-height: 18px;
  white-space: nowrap;
  overflow: hidden;
...
.finished-row-warning :deep(.t-alert__icon) {
  font-size: 14px;
}
.finished-row-warning :deep(.t-alert__icon),
.finished-row-warning :deep(.t-alert__close) {
  display: flex;
  align-items: center;
