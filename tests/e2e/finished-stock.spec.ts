    ...
    route.fulfill({
      json: {
        ...
        data: [
          ...
          { id: 2, categoryId: 6, attributeId: 2, attributeRole: 'product', requiredFlag: true, status: 'enabled' },
          { id: 3, categoryId: 5, attributeId: 2, attributeRole: 'product', requiredFlag: false, status: 'enabled' },
        ],
      },
    }),
  );
...
test('refreshes configured finished prices and calculates dynamic specification and batch prices', async ({ page }) => {
  ...
  const manualCoefficient = manualCell.getByPlaceholder('价格系数');
  ...
  await expect(manualCell.getByText('请输入正确的价格系数', { exact: true })).toBeVisible();
  await manualCoefficient.fill('');
  await manualCoefficient.press('Tab');
  await expect(manualCell.getByText('请输入价格系数', { exact: true })).toBeVisible();

  await expect(table.getByRole('columnheader', { name: '未配置级别*', exact: true })).toBeVisible();
  await expect(manualCell.getByPlaceholder('价格系数')).toHaveValue('');
  await expect(manualCell.getByPlaceholder('价格', { exact: true })).toHaveValue('');
  await manualCell.getByPlaceholder('价格系数').fill('1.3');
  ...
  await expect(configuredCell.getByPlaceholder('价格系数')).toHaveValue('1.50');
  ...
  await batchPrice.getByPlaceholder('价格系数').fill('2');
  await page.getByRole('button', { name: '保存', exact: true }).click();
  await expect(configuredCell.getByPlaceholder('价格', { exact: true })).toHaveValue('200.00');
  await expect(configuredCell.getByPlaceholder('价格系数')).toHaveValue('2.00');
});
...
test('uses only template-bound role attributes and builds dynamic sales specifications', async ({ page }) => {
  ...
  await page.route('**/api/admin/category-attributes', (route) =>
    route.fulfill(
      respond([
        { id: 1, categoryId: 5, attributeId: 1, attributeRole: 'product', requiredFlag: true },
        { id: 2, categoryId: 5, attributeId: 2, attributeRole: 'sales', requiredFlag: true, skuFlag: true },
  ...
  await expect(page.locator('#finished-product-sales')).toContainText('销售测试属性');
  await page.getByRole('button', { name: '创建规格', exact: true }).click();
  ...
  const salesInput = table.locator('tbody tr td').nth(1).getByRole('textbox');
  await expect(salesInput.locator('..')).toHaveClass(/t-is-error/);
  await salesInput.fill('大号');
  ...
  await page.getByRole('button', { name: '确认创建', exact: true }).click();
  await expect(table.locator('tbody tr td').first().getByRole('textbox')).toHaveValue('小号');
});
